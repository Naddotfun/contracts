...
contract CoreCreateTest is Test, SetUp {
    ...
    function testCreateCurveSuccess() public {
        ...
        (uint256 vNad, uint256 vToken) = BondingCurve(payable(curveAddress)).getVirtualReserves();
    ...
    function testCreateCurveWithInitialLiquidity() public {
        ...
        CORE.createCurve{value: initialNad + fee + DEPLOY_FEE}(
            TRADER_A, "Test Token", "TEST", "test.url", initialNad, fee
        );
    ...
    function testCreateCurveInsufficientDeployFee() public {
        ...
        CORE.createCurve{value: DEPLOY_FEE - 1}(TRADER_A, "Test Token", "TEST", "test.url", 0, 0);
    ...
    function testCreateCurveInvalidInitialAmount() public {
        ...
        CORE.createCurve{value: initialNative + fee + DEPLOY_FEE - 1}(
            TRADER_A, "Test Token", "TEST", "test.url", initialNative, fee
        );
...
contract CoreBuyTest is Test, SetUp {
    ...
    function testBuySuccess() public {
        ...
        // Execute buy
        CORE.buy{value: amountIn + fee}(amountIn, fee, address(MEME_TOKEN), to, deadline);
    ...
    function testBuyFailInvalidNadAmount() public {
        ...
        vm.expectRevert(bytes(ERR_CORE_INVALID_SEND_NATIVE));
        CORE.buy{value: amountIn}(amountIn, fee, address(MEME_TOKEN), OWNER, deadline);
        vm.stopPrank();
    }
    ...
    function testBuyFailZeroAmountIn() public {
        ...
        CORE.buy{value: fee}(amountIn, fee, address(MEME_TOKEN), OWNER, deadline);
        vm.stopPrank();
    }
    ...
    function testBuyFailExpiredDeadline() public {
        ...
        CORE.buy{value: amountIn + fee}(amountIn, fee, address(MEME_TOKEN), OWNER, deadline);
        vm.stopPrank();
    }
    ...
    function testProtectBuySuccess() public {
        ...
        uint256 amountOutMin = BondingCurveLibrary.getAmountOut(amountIn, CURVE.getK(), virtualNative, virtualToken);

        address to = OWNER;
        uint256 deadline = block.timestamp + 1 hours;

        // Execute buy with minimum amount out
        CORE.protectBuy{value: amountIn + fee}(amountIn, amountOutMin - 1, fee, address(MEME_TOKEN), to, deadline);
    ...
    function testProtectBuyFailInvalidNadAmount() public {
        ...
        (uint256 virtualNative, uint256 virtualToken) = CURVE.getVirtualReserves();
        uint256 amountOut = BondingCurveLibrary.getAmountOut(amountIn, CURVE.getK(), virtualNative, virtualToken);
    ...
    function testProtectBuyFailInvalidAmountOut() public {
        ...
        CORE.protectBuy{value: amountIn + fee}(amountIn, amountOutMin, fee, address(MEME_TOKEN), OWNER, deadline);
        vm.stopPrank();
    }
    ...
    function testExactOutBuySuccess() public {
        ...
        (uint256 virtualNative, uint256 virtualToken) = CURVE.getVirtualReserves();
        uint256 amountIn = BondingCurveLibrary.getAmountIn(amountOut, CURVE.getK(), virtualNative, virtualToken);
        ...
        CORE.exactOutBuy{value: amountInMax}(amountInMax, amountOut, address(MEME_TOKEN), OWNER, deadline);
    ...
    function testExactOutBuyFailInvalidNadAmount() public {
        ...
        vm.expectRevert(bytes(ERR_CORE_INVALID_SEND_NATIVE));
        CORE.exactOutBuy{value: amountInMax - 1}(amountInMax, amountOut, address(MEME_TOKEN), OWNER, deadline);
        vm.stopPrank();
    }
...
contract CoreSellTest is Test, SetUp {
    ...
    function testSellSuccess() public {
        ...
        // 예상 출력값 계산
        uint256 expectedAmountOut = BondingCurveLibrary.getAmountOut(amountIn, k, virtualToken, virtualNative);
        (uint8 denominator, uint16 numerator) = CURVE.getFeeConfig();
        uint256 expectedFee = BondingCurveLibrary.getFeeAmount(expectedAmountOut, denominator, numerator);
        ...
        // 결과 검증
        assertEq(MEME_TOKEN.balanceOf(TRADER_A), initialTokenBalance - amountIn, "Invalid token balance after sell");
        assertEq(
            TRADER_A.balance, initialNadBalance + expectedAmountOut - expectedFee, "Invalid NAD balance after sell"
        );
    }
    ...
    function testProtectSellSuccess() public {
        ...
        // protectSell 함수 실행
        CORE.protectSell(amountIn, amountOutMin, address(MEME_TOKEN), TRADER_A, deadline);
        vm.stopPrank();
    ...
    function testProtectSellFailExpiredDeadline() public {
        ...
        vm.expectRevert(bytes(ERR_CORE_EXPIRED));
        CORE.protectSell(amountIn, 0, address(MEME_TOKEN), address(this), deadline);
    }
    ...
    function testSellPermitSuccess() public {
        ...
        bytes32 structHash = keccak256(abi.encode(PERMIT_TYPEHASH, signer, address(CORE), amountIn, nonce, deadline));
        ...
        // sellPermit 함수 실행
        CORE.sellPermit(amountIn, address(MEME_TOKEN), signer, signer, deadline, v, r, s);
        vm.stopPrank();

        // 결과 검증
        assertEq(MEME_TOKEN.balanceOf(signer), initialTokenBalance - amountIn, "Invalid token balance after sell");
        assertTrue(signer.balance > initialNadBalance, "NAD balance should increase after sell");
    }
    ...
    function testSellPermitFailExpiredPermitDeadline() public {
        ...
        bytes32 structHash = keccak256(abi.encode(PERMIT_TYPEHASH, TRADER_A, address(CORE), amountIn, nonce, deadline));
        ...
        CORE.sellPermit(amountIn, address(MEME_TOKEN), TRADER_A, TRADER_A, deadline, v, r, s);
    }
    ...
    function testProtectSellPermitSuccess() public {
        ...
        CORE.protectSellPermit(amountIn, amountOutMin, address(MEME_TOKEN), signer, signer, deadline, v, r, s);
        vm.stopPrank();
    ...
    function testProtectSellPermitFailInsufficientOutput() public {
        ...
        uint256 amountOutMin = expectedAmountOut + 1 ether;
        // TRADER_A의 private key 가져오기
        uint256 privateKey = uint256(keccak256(abi.encodePacked("TRADER_A")));
        address signer = vm.addr(privateKey);
        ...
        bytes32 structHash = keccak256(abi.encode(PERMIT_TYPEHASH, signer, address(CORE), tokenAmount, nonce, deadline));
        ...
        vm.expectRevert(bytes(ERR_CORE_INVALID_AMOUNT_OUT));
        CORE.protectSellPermit(tokenAmount, amountOutMin, address(MEME_TOKEN), signer, signer, deadline, v, r, s);
    }
    ...
    function testExactOutSellSuccess() public {
        ...
        // 예상 출력값 계산
        uint256 amountOut = BondingCurveLibrary.getAmountOut(initialTokenBalance, k, virtualToken, virtualNative) - 100;

        uint256 fee = amountOut / 100;

        vm.deal(TRADER_A, fee);
        // exactOutSell 함수 실행
        CORE.exactOutSell{value: fee}(
            initialTokenBalance, amountOut, address(MEME_TOKEN), TRADER_A, block.timestamp + 1
        );

        // 결과 검증
        assertTrue(MEME_TOKEN.balanceOf(TRADER_A) < initialTokenBalance, "Token balance should decrease after sell");
        assertEq(TRADER_A.balance - initialNadBalance, amountOut, "NAD balance should increase by exact amount");
        vm.stopPrank();
    }

    function testExactOutSellFailExcessiveAmountOut() public {
        Buy(TRADER_A, 1 ether);
        ...
        uint256 expectedAmountOut = BondingCurveLibrary.getAmountOut(tokenAmount, k, virtualToken, virtualNative);

        uint256 amountIn = tokenAmount - 1;
        uint256 fee = expectedAmountOut / 100;
        vm.deal(TRADER_A, fee);
        vm.startPrank(TRADER_A);
        MEME_TOKEN.approve(address(CORE), tokenAmount);

        vm.expectRevert(bytes(ERR_CORE_INVALID_AMOUNT_IN_MAX));
        CORE.exactOutSell{value: fee}(amountIn, expectedAmountOut, address(MEME_TOKEN), TRADER_A, block.timestamp + 1);
        vm.stopPrank();
    }
    ...
    function testExactOutSellPermitSuccess() public {
        ...
        (uint256 virtualNative, uint256 virtualToken) = CURVE.getVirtualReserves();
        //amountOut 을 계산하기
        uint256 amountOut = BondingCurveLibrary.getAmountOut(tokenAmount, CURVE.getK(), virtualToken, virtualNative) - 1;

        uint256 fee = (amountOut) / 100;
        vm.deal(signer, fee);
        // exactOutSellPermit 함수 실행
        CORE.exactOutSellPermit{value: fee}(
            tokenAmount, amountOut, address(MEME_TOKEN), signer, signer, deadline, v, r, s
        );
        vm.stopPrank();

        // 결과 검증
        assertTrue(MEME_TOKEN.balanceOf(signer) < initialTokenBalance, "Token balance should decrease after sell");
        assertEq(signer.balance, amountOut, "NAD balance should exact amountOut");
    }
    ...
    function testExactOutSellPermitFailExcessiveInput() public {
        ...
        uint256 amountOut = BondingCurveLibrary.getAmountOut(tokenAmount, CURVE.getK(), virtualToken, virtualNative);

        uint256 wantedAmountOut = amountOut + 1 ether;
        uint256 fee = (wantedAmountOut) / 100;
        vm.deal(signer, fee);

        vm.expectRevert(bytes(ERR_CORE_INVALID_AMOUNT_IN_MAX));
        CORE.exactOutSellPermit{value: fee}(
            tokenAmount, wantedAmountOut, address(MEME_TOKEN), signer, signer, deadline, v, r, s
        );
    }
}
