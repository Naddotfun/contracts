...
contract BondingCurveLibraryTest is Test, SetUp {
    ...
    function testGetAmountOut() public pure {
        uint256 amountOut = BondingCurveLibrary.getAmountOut(AMOUNT_IN, TEST_K, RESERVE_IN, RESERVE_OUT);

        // Manual calculation for verification
        uint256 expectedAmountOut = RESERVE_OUT - (TEST_K / (RESERVE_IN + AMOUNT_IN)) - 1;
        assertEq(amountOut, expectedAmountOut, "Amount out calculation mismatch");

        // Verify the amount out is reasonable
        assertTrue(amountOut < RESERVE_OUT, "Amount out should be less than reserve out");
    ...
    function testGetAmountIn() public {
        ...
        uint256 amountIn = BondingCurveLibrary.getAmountIn(desiredAmountOut, TEST_K, RESERVE_IN, RESERVE_OUT);

        // Verify the calculated amount in produces the desired amount out
        uint256 resultingAmountOut = BondingCurveLibrary.getAmountOut(amountIn, TEST_K, RESERVE_IN, RESERVE_OUT);

        // Allow for up to 7 wei difference due to rounding
        assertApproxEqAbs(resultingAmountOut, desiredAmountOut, 100, "Amount in/out calculation mismatch");

        // Test case 2: Small amount (1 wei)
        uint256 smallAmountOut = 1;
        uint256 smallAmountIn = BondingCurveLibrary.getAmountIn(smallAmountOut, TEST_K, RESERVE_IN, RESERVE_OUT);
        ...
        vm.expectRevert();
        BondingCurveLibrary.getAmountIn(largeAmountOut, TEST_K, RESERVE_IN, RESERVE_OUT);
        ...
        uint256 testAmountIn = BondingCurveLibrary.getAmountIn(testAmountOut, testK, testReserveIn, testReserveOut);

        // Verify k remains constant
        uint256 newK = (testReserveIn + testAmountIn) * (testReserveOut - testAmountOut);
        assertEq(newK, testK, "K should remain constant after getAmountIn calculation");
    }
    ...
    function testGetAmountInWithInvalidAmountOut() public {
        ...
        BondingCurveLibrary.getAmountIn(invalidAmountOut, TEST_K, RESERVE_IN, RESERVE_OUT);
    }
    ...
    function testGetAmountAndFee() public {
        ...
        (uint256 fee, uint256 adjustedAmount) = BondingCurveLibrary.getAmountAndFee(address(CURVE), testAmount);
        ...
        assertEq(adjustedAmount, testAmount - fee, "Adjusted amount calculation mismatch");
    }
    ...
    function testGetFeeAmount() public pure {
        ...
        uint256 fee = BondingCurveLibrary.getFeeAmount(amount, denominator, numerator);
    ...
    function testGetCurveData() public {
        ...
        (address curve, uint256 virtualNative, uint256 virtualToken, uint256 k) =
            BondingCurveLibrary.getCurveData(address(BONDING_CURVE_FACTORY), address(MEME_TOKEN));

        // Verify returned data
        assertEq(curve, address(CURVE), "Curve address mismatch");
        (uint256 expectedVNad, uint256 expectedVToken) = CURVE.getVirtualReserves();
        ...
        (virtualNative, virtualToken, k) = BondingCurveLibrary.getCurveData(address(CURVE));
        assertEq(virtualNative, expectedVNad, "Virtual NAD mismatch (direct)");
        assertEq(virtualToken, expectedVToken, "Virtual token mismatch (direct)");
    ...
    function testGetFeeConfig() public {
        ...
        (uint8 denominator, uint16 numerator) = BondingCurveLibrary.getFeeConfig(address(CURVE));
    ...
    function testGetCurve() public {
        ...
        address curve = BondingCurveLibrary.getCurve(address(BONDING_CURVE_FACTORY), address(MEME_TOKEN));
    ...
    function testGetVirtualReserves() public {
        ...
        (uint256 virtualNative, uint256 virtualToken) = BondingCurveLibrary.getVirtualReserves(address(CURVE));
    ...
    function testEdgeCases() public {
        ...
        uint256 amountOut = BondingCurveLibrary.getAmountOut(smallAmountIn, TEST_K, RESERVE_IN, RESERVE_OUT);
        ...
        BondingCurveLibrary.getAmountOut(largeAmountIn, TEST_K, RESERVE_IN, RESERVE_OUT);
        ...
        uint256 resultOut = BondingCurveLibrary.getAmountOut(testAmountIn, k, reserveIn, reserveOut);

        uint256 calculatedAmountIn = BondingCurveLibrary.getAmountIn(resultOut, k, reserveIn, reserveOut);

        // Should be equal or differ by at most 1 due to rounding
        assertTrue(
            calculatedAmountIn == testAmountIn || calculatedAmountIn == testAmountIn + 1,
        ...
        uint256 exactOut = BondingCurveLibrary.getAmountOut(exactAmountIn, exactK, exactReserveIn, exactReserveOut);

        // Verify k remains constant
        uint256 newK = (exactReserveIn + exactAmountIn) * (exactReserveOut - exactOut);
    ...
    function testFuzzGetAmountOut(uint256 amountIn) public pure {
        ...
        uint256 amountOut = BondingCurveLibrary.getAmountOut(amountIn, TEST_K, RESERVE_IN, RESERVE_OUT);

        assertTrue(amountOut < RESERVE_OUT, "Amount out should be less than reserve");
        assertTrue(amountOut > 0, "Amount out should be greater than 0");
    }
}
