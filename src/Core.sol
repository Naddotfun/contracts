...
contract Core is ICore {
    ...
    function buyExactAmountOutWNad(
        ...
        require(amountOut > 0, ERR_CORE_INVALID_AMOUNT_OUT);
        uint256 allowance = IERC20(WNAD).allowance(msg.sender, address(this));
        require(allowance >= amountInMax, ERR_CORE_INVALID_ALLOWANCE);
        ...
        require(amountInMax >= amountIn + fee, ERR_CORE_INVALID_AMOUNT_IN_MAX);
        checkFee(curve, amountIn, fee);
    ...
    function buyExactAmountOutWNadPermit(
        ...
        require(amountOut > 0, ERR_CORE_INVALID_AMOUNT_IN);
