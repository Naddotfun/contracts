...
contract Lock is ILock {
    ...
    function initialize(address _bondingCurveFactory, uint256 _defaultLockTime) external onlyOwner {
    ...
    function unlock(address token, address account) external {
        ...
        address curve = IBondingCurveFactory(bondingCurveFactory).getCurve(token);
