...
import {MintPartyV2} from "./MintPartyV2.sol";
 ...
 */
contract MintPartyFactoryV2 is IMintPartyFactory {
    ...
    function create(
        ...
        if (existingParty != address(0)) {
            require(
                IMintPartyV2(existingParty).getFinished(),
                ERR_MINT_PARTY_FACTORY_NOT_FINISHED
            );
        }

        // Create new MintParty instance
        MintPartyV2 party = new MintPartyV2(
            account,
            core,
            WNad,
            lock,
        ...
        IMintPartyV2(party).deposit{value: fundingAmount}(account);
